...
const edgeFeatures = [
  { icon: ShieldCheck, title: 'Verified Information', desc: 'Directly sourced from university administration.', tint: 'bg-orange-50 dark:bg-orange-900/15', color: 'text-orange-500' },
  ...
  { icon: Users, title: 'Student Community', desc: 'Connect with peers and alumni.', tint: 'bg-orange-50 dark:bg-orange-900/15', color: 'text-orange-500' },
...
function PersonalizedSearchModal({ open, onClose }) {
  ...
  const inputClass =
    'w-full rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-dark-bg px-4 py-3 text-sm font-semibold text-slate-900 dark:text-white placeholder:text-slate-400 focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 outline-none transition-all';
  const labelClass =
    ...
    <AnimatePresence>
      {open && (
        <div className="fixed inset-0 z-[130] flex items-center justify-center px-4">
          ...
          >
            <div className="relative overflow-hidden bg-primary-dark px-8 py-8 text-white">
              <div className="absolute -right-8 -top-10 h-40 w-40 rounded-full bg-orange-500/30 blur-2xl" />
              <div className="relative flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-2 max-w-md text-sm text-white/80">Tell us a little about you and we’ll match universities to your state, stream, budget and marks.</p>
                </div>
                <button onClick={onClose} aria-label="Close" className="rounded-full p-1 text-white/70 transition-colors hover:bg-white/10 hover:text-white">
                  <X className="h-6 w-6" />
                </button>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-5 p-8 md:grid-cols-2">
              ...
              <div className="mt-2 flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between md:col-span-2">
                <p className="flex items-center gap-2 text-xs text-slate-400">
                  <ShieldCheck className="h-4 w-4 text-orange-500" /> We only use this to match colleges. No spam.
                </p>
                <button type="submit" disabled={submitting}
                  className="inline-flex items-center justify-center gap-2 rounded-xl bg-orange-500 px-8 py-3.5 text-sm font-bold text-white shadow-lg shadow-orange-500/30 transition-all hover:bg-orange-600 active:scale-95 disabled:opacity-60">
  ...
  return (
    <div className="mb-8 flex items-center justify-between">
      <h2 className="flex items-center gap-3 text-2xl font-bold text-slate-900 dark:text-white">
        <span className="h-8 w-2 rounded-full bg-orange-500" /> {children}
      </h2>
      {action}
    </div>
  );
...
function CenteredHeading({ eyebrow, title }) {
  return (
    <div className="mb-12 text-center">
      <p className="mb-3 text-xs font-bold uppercase tracking-[0.3em] text-orange-500">{eyebrow}</p>
...
function DiscoveryTile({ tile, tall, onClick }) {
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-primary-dark/45 transition-colors duration-300 group-hover:bg-primary-dark/65" />
          ...
          <div className="mt-8 max-w-xl">
            <form onSubmit={handleSearch} className="relative">
              <div className="relative flex overflow-hidden rounded-none bg-white p-1 shadow-[0_20px_50px_rgba(0,0,0,0.3)]">
                ...
                <button type="submit" className="whitespace-nowrap rounded-none bg-orange-500 px-8 font-bold text-white shadow-lg transition-all hover:bg-orange-600 active:scale-95 md:px-10">Search</button>
              </div>
                ...
                <div className="absolute left-0 right-0 top-[calc(100%+12px)] z-30 overflow-hidden rounded-none border border-slate-200 bg-white p-2 text-left shadow-[0_24px_60px_rgba(15,23,42,0.28)]">
                  {quickSearchSuggestions.map((suggestion) => (
                    <button key={`${suggestion.label}-${suggestion.sublabel}`} type="button" onClick={suggestion.action}
                      className="flex w-full items-center justify-between gap-4 rounded-none px-4 py-3 transition-colors hover:bg-slate-100 hover:text-orange-500">
                      <div>
                        <p className="text-sm font-bold text-slate-900 group-hover:text-orange-500">{suggestion.label}</p>
          ...
          <form onSubmit={(e) => { e.preventDefault(); setShowPersonalized(true); }} className="space-y-4">
            <div>
              ...
              <input type="text" placeholder="Enter your full name"
                className="w-full rounded-none border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm font-medium text-slate-900 placeholder:text-slate-400 focus:border-orange-500 focus:bg-white focus:outline-none focus:ring-1 focus:ring-orange-500" />
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select className="w-full rounded-none border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm font-medium text-slate-700 focus:border-orange-500 focus:bg-white focus:outline-none focus:ring-1 focus:ring-orange-500">
            ...
            <button type="submit" className="mt-2 w-full rounded-none bg-orange-500 py-3 font-bold text-white shadow-md transition-all hover:bg-orange-600 active:scale-[0.98]">
  ...
  <div className="absolute bottom-4 left-1/2 z-30 flex -translate-x-1/2 gap-2">
    {featuredUniversities.map((_, i) => (
      <button key={i} onClick={() => setCurrentSlide(i)} aria-label={`Slide ${i + 1}`}
        className={`h-1.5 rounded-none transition-all duration-500 ${i === currentSlide % featuredUniversities.length ? 'w-8 bg-orange-500' : 'w-2 bg-white/40'}`} />
    ))}
  </div>
</section>
      {/* ======================== QUICK STATS ======================== */}
        ...
        <div className="flex flex-wrap justify-between gap-8 rounded-2xl border border-slate-100 bg-white p-8 shadow-lg dark:border-white/5 dark:bg-dark-card md:p-10">
          {displayStats.map((s, i) => (
            <div key={i} className="group relative flex min-w-[150px] flex-1 cursor-default items-center justify-center gap-4 overflow-hidden rounded-2xl p-4 md:justify-start">
              <div className="absolute inset-0 z-0 origin-left scale-x-0 bg-gradient-to-br from-orange-700 to-orange-500 transition-transform duration-500 ease-out group-hover:scale-x-100" />
              <div className="relative z-10 flex w-full items-center justify-center gap-4 md:justify-start">
                <div className="rounded-2xl bg-orange-50 p-3 transition-colors duration-500 group-hover:bg-white/20 dark:bg-orange-900/20">
                  <s.icon className="h-6 w-6 text-orange-500 transition-colors duration-500 group-hover:text-white" />
                </div>
                <div>
                  ...
                  <p className="mt-1 text-[10px] font-bold uppercase tracking-widest text-slate-400 transition-colors duration-500 group-hover:text-orange-100">{s.label}</p>
                </div>
              </div>
            </div>
          ))}
  ...
  >
    ...
    <div className="flex flex-col gap-3">
      ...
      <section className="mx-auto max-w-7xl px-4 py-12">
        <SectionHeading
          action={
            <Link to="/universities" className="group inline-flex items-center gap-1 text-sm font-bold text-orange-500 hover:text-orange-600">
              Explore all <ChevronRight className="h-4 w-4 transition-transform group-hover:translate-x-1" />
            </Link>
          }
        >
          <span className="inline-flex items-center gap-2"><CheckCircle2 className="h-6 w-6 text-orange-500" /> Featured Partner Universities</span>
        </SectionHeading>
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            {universities.map((u, i) => (
              <motion.div key={i} whileHover={{ y: -5 }}>
                <Link to={getUniversityPath(u)} className="group relative block overflow-hidden rounded-2xl border border-slate-100 bg-white p-6 transition-all hover:border-transparent hover:shadow-lg hover:shadow-orange-500/20 dark:border-white/5 dark:bg-dark-card">
                  <div className="absolute inset-0 z-0 origin-left scale-x-0 bg-gradient-to-br from-orange-700 to-orange-500 transition-transform duration-500 ease-out group-hover:scale-x-100" />
                  <div className="relative z-10 flex items-center gap-6">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mb-4 mt-1 flex items-center gap-2 text-xs text-slate-400 transition-colors group-hover:text-white/90">
                        <MapPin className="h-3 w-3 shrink-0 text-orange-500 transition-colors group-hover:text-white" />
          ...
          {mainStreams.map((stream, i) => (
            <motion.div key={i} variants={itemVariants} className="w-[160px] flex-shrink-0 snap-start sm:w-[180px] lg:w-auto">
              <Link to={getStreamLink(stream.name)}
                ...
                <div className="absolute inset-0 z-0 origin-left scale-x-0 bg-gradient-to-r from-orange-500 to-primary transition-transform duration-300 ease-out group-hover:scale-x-100" />
      ...
      <div className="mx-auto grid max-w-7xl grid-cols-1 gap-8 px-4 py-8 lg:grid-cols-12">
        ...
        <div className="space-y-12 lg:col-span-8">
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
              {[
                ...
                <motion.div key={i} whileHover={{ y: -4 }}
                  className="group relative overflow-hidden rounded-2xl border border-slate-100 bg-white p-6 text-slate-900 shadow-sm transition-all hover:border-orange-500/40 hover:shadow-lg dark:border-white/5 dark:bg-dark-card dark:text-white">
                  <div className="relative z-10 flex h-full flex-col">
                    ...
                    <h3 className="mb-2 mt-4 text-xl font-bold transition-colors group-hover:text-orange-500">{c.title}</h3>
                    <p className="mb-6 flex-grow text-sm text-slate-500 dark:text-slate-400">{c.desc}</p>
                    <Link to={c.to} className="mt-auto flex w-full items-center justify-center rounded-xl bg-orange-500 py-3 text-sm font-bold text-white shadow-sm transition-all hover:bg-orange-600">{c.cta}</Link>
                  </div>
                </motion.div>
              ))}
            </div>
          ...
          {/* Need help */}
          <section className="relative overflow-hidden rounded-2xl bg-primary-dark p-6 text-white md:p-8">
            <div className="absolute -right-10 -top-10 h-48 w-48 rounded-full bg-orange-500/20 blur-3xl" />
            <div className="relative z-10 flex flex-col items-start justify-between gap-6 md:flex-row md:items-center">
              <div className="flex items-start gap-4">
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-orange-500/20"><LifeBuoy className="h-6 w-6 text-orange-400" /></div>
                <div>
                  <h3 className="text-xl font-bold">Need help choosing?</h3>
                  <p className="mt-2 max-w-md text-sm text-white/80">Get a shortlist matched to your marks, budget and preferred state — or talk to a counsellor directly.</p>
                </div>
              </div>
              <div className="flex w-full shrink-0 gap-3 md:w-auto">
                <button onClick={() => setShowPersonalized(true)} className="flex-1 rounded-xl bg-orange-500 px-5 py-3 text-sm font-bold text-white shadow-sm transition-all hover:bg-orange-600 md:flex-none">Get matched</button>
                <button onClick={openChat} className="flex-1 rounded-xl border border-white/20 bg-white/10 px-5 py-3 text-sm font-bold text-white transition-all hover:bg-white/20 md:flex-none">Ask Mitra AI</button>
              </div>
            </div>
          </section>
          ...
          <section>
            <SectionHeading
              action={
                <Link to="/universities" className="group flex items-center gap-1 text-sm font-bold text-orange-500 hover:text-orange-600">
                  Explore all <ChevronRight className="h-4 w-4 transition-transform group-hover:translate-x-1" />
                </Link>
              }
            >
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                {universities.slice(0, 4).map((u, i) => (
                  <motion.div key={i} whileHover={{ y: -3 }}>
                    <Link to={getUniversityPath(u)} className="group flex items-center gap-4 rounded-2xl border border-slate-100 bg-white p-4 transition-all hover:border-orange-500/40 hover:shadow-md dark:border-white/5 dark:bg-dark-card">
                      ...
                      <div className="min-w-0 flex-1">
                        <h4 className="line-clamp-1 text-sm font-bold text-slate-900 transition-colors group-hover:text-orange-500 dark:text-white">{u.name}</h4>
                        <p className="mt-0.5 flex items-center gap-1 text-xs text-slate-400"><MapPin className="h-3 w-3 text-orange-500" /> {u.state || 'India'}</p>
                      </div>
                      <ChevronRight className="h-4 w-4 shrink-0 text-slate-300 transition-transform group-hover:translate-x-1 group-hover:text-orange-500" />
                    </Link>
                  </motion.div>
                ))}
              </div>
            ...
            {universities.length > 0 && (
              <div className="mt-8 text-center">
                <Link to="/universities" className="group inline-flex items-center gap-3 rounded-xl border-2 border-orange-500 bg-white px-8 py-3.5 text-sm font-bold text-orange-500 transition-all hover:bg-orange-500 hover:text-white dark:bg-dark-card">
          ...
          <section>
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              {(universities.length ? universities.slice(0, 2) : featuredUniversities.slice(0, 2)).map((u, i) => (
                <motion.div key={i} whileHover={{ y: -4 }}>
                  <Link to={getUniversityPath(u)} className="group flex items-center gap-5 rounded-2xl border border-slate-100 bg-white p-5 transition-all hover:border-orange-500/40 hover:shadow-md dark:border-white/5 dark:bg-dark-card">
                    ...
                    <div className="min-w-0 flex-1">
                      <h4 className="line-clamp-1 text-base font-bold text-slate-900 transition-colors group-hover:text-orange-500 dark:text-white">{u.name}</h4>
                      <p className="mt-1 flex items-center gap-1 text-xs text-slate-400"><MapPin className="h-3 w-3 text-orange-500" /> {u.location || u.state || 'India'}</p>
        ...
        <div className="space-y-6 lg:col-span-4">
          ...
          <section className="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm dark:border-white/5 dark:bg-dark-card">
            <div className="flex items-center justify-between bg-primary-dark p-5 text-white">
              <h2 className="text-base font-bold">Admission Updates</h2><ClipboardList className="h-4 w-4" />
            </div>
            <div className="divide-y divide-slate-100 dark:divide-white/5">
              {exams.length > 0 ? exams.map((ex, i) => (
                <Link to="/exams" key={i} className="group block p-4 transition-all hover:bg-slate-50 dark:hover:bg-white/5">
                  <span className="text-[9px] font-bold uppercase tracking-wider text-orange-500">{ex.stream || ex.category || 'Exam'}</span>
                  <h4 className="mt-1 line-clamp-2 text-sm font-bold transition-colors group-hover:text-orange-500">{ex.name || ex.title}</h4>
                </Link>
              )) : (
                <div className="p-6 text-center text-sm text-slate-400">No admission updates yet.</div>
              )}
              <Link to="/exams" className="block bg-slate-50 py-3.5 text-center text-xs font-bold text-orange-500 transition-colors hover:bg-slate-100 hover:text-orange-600 dark:bg-white/5">View all notifications</Link>
            </div>
          </section>

          {/* Latest news / alerts */}
          <section className="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm dark:border-white/5 dark:bg-dark-card">
            <div className="flex items-center justify-between bg-gradient-to-r from-orange-500 to-primary p-5 text-white">
              <h2 className="text-base font-bold">Latest News &amp; Alerts</h2><Bell className="h-4 w-4" />
            </div>
            <div className="divide-y divide-slate-100 dark:divide-white/5">
              {news.length > 0 ? news.map((n, i) => (
                <div key={i} className="group cursor-pointer p-4 transition-all hover:bg-slate-50 dark:hover:bg-white/5">
                  <span className="text-[9px] font-bold uppercase tracking-wider text-orange-500">{n.category || 'News'}</span>
                  <h4 className="mt-1.5 line-clamp-2 text-sm font-bold transition-colors group-hover:text-orange-500">{n.title}</h4>
                </div>
              )) : (
          ...
          <section className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm dark:border-white/5 dark:bg-dark-card">
            <div className="mb-5 flex items-center gap-3">
              <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-orange-50 dark:bg-white/5"><MessageSquare className="h-4 w-4 text-orange-500" /></div>
            ...
            <div className="space-y-3.5">
              {questions.length > 0 ? questions.map((q, i) => (
                <div key={i} className="group cursor-pointer border-b border-slate-100 pb-3 last:border-none dark:border-white/5" onClick={openChat}>
                  <h4 className="text-sm font-bold transition-colors group-hover:text-orange-500">{q.title || q.content}</h4>
                </div>
              )) : (
                <p className="text-sm text-slate-400">Be the first to ask a question.</p>
              )}
            </div>
            <button onClick={openChat} className="mt-6 w-full rounded-xl bg-orange-500 py-3.5 text-xs font-bold text-white shadow-md transition-all hover:bg-orange-600">ASK A QUESTION</button>
          </section>

          {/* Stay ahead */}
          <section className="rounded-2xl bg-primary-dark p-6 text-white">
            <h2 className="mb-2 text-xl font-bold">Stay Ahead</h2>
            <p className="mb-6 text-xs text-white/70">Get the latest admission alerts and entrance-exam tips in your inbox.</p>
            <div className="flex gap-2">
              <input type="email" placeholder="Email address" className="min-w-0 flex-1 rounded-xl border border-white/15 bg-white/5 px-3.5 text-sm text-white placeholder:text-white/40 outline-none focus:border-orange-500" />
              <button className="rounded-xl bg-orange-500 px-5 py-2.5 text-xs font-bold text-white transition-colors hover:bg-orange-600">JOIN</button>
            </div>
          </section>
      ...
      <section className="mx-auto max-w-7xl px-4 py-8">
        <div className="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm dark:border-white/5 dark:bg-dark-card">
          <div className="grid gap-8 p-6 md:p-8 lg:grid-cols-[1.4fr_0.9fr] lg:items-center">
            <div>
              <p className="mb-2 text-[10px] font-bold uppercase tracking-widest text-orange-500">Contact &amp; feedback</p>
            ...
            <div className="rounded-2xl bg-primary-dark p-6 text-white shadow-md">
              <div className="flex items-start gap-4">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-orange-500/20"><MessageSquare className="h-5 w-5 text-orange-400" /></div>
                <div>
                  <p className="text-base font-bold">Share feedback</p>
                  <p className="mt-1 text-xs leading-relaxed text-white/70">Tell us what’s working, what’s confusing, or what to improve next.</p>
                </div>
              </div>
              <button onClick={() => setShowFeedback(true)} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-orange-500 px-4 py-3 text-sm font-bold text-white shadow-md transition-all hover:bg-orange-600">
      ...
      <section className="mx-auto max-w-7xl px-4 py-16">
            ...
            <motion.div key={i} variants={itemVariants} whileHover={{ y: -6 }}
              ...
              <div className="mb-4 flex gap-1">
                {Array.from({ length: 5 }).map((_, s) => (
                  <Star key={s} className={`h-4 w-4 ${s < (r.rating || 5) ? 'fill-orange-500 text-orange-500' : 'text-slate-200 dark:text-white/15'}`} />
                ))}
              </div>
              ...
              <div className="mt-6 flex items-center gap-3 border-t border-slate-100 pt-5 dark:border-white/5">
                {r.avatarUrl ? (
                  ...
                  <div className="flex h-11 w-11 items-center justify-center rounded-full bg-primary-dark text-sm font-bold text-white">
        ...
        <div className="mt-10 text-center">
          <button onClick={() => setShowFeedback(true)} className="inline-flex items-center gap-2 rounded-xl border-2 border-orange-500 px-8 py-3.5 text-sm font-bold text-orange-500 transition-all hover:bg-orange-500 hover:text-white">
        ...
        {showFeedback && (
          <div className="fixed inset-0 z-[120] flex items-center justify-center px-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.97, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.97, y: 10 }}
              ...
              <div className="flex items-center justify-between border-b border-slate-800 bg-primary-dark p-5 md:p-6">
                <h2 className="text-xl font-bold uppercase tracking-tight text-white md:text-2xl">Feedback Form</h2>
                <button onClick={() => setShowFeedback(false)} className="text-white/60 transition-colors hover:text-white"><X className="h-6 w-6" /></button>
              </div>
                  ...
                  <div className="space-y-5 lg:col-span-8">
                    <div className="grid grid-cols-1 gap-4">
                      <div>
                        ...
                        <input name="name" required type="text" placeholder="Enter your name" className="w-full rounded-xl border border-slate-200 p-3.5 text-sm outline-none transition-all focus:border-orange-500" />
                      </div>
                      <div>
                        <label className="mb-1 block text-[10px] font-bold uppercase tracking-wider text-slate-500">Mobile</label>
                        <input name="mobile" type="text" placeholder="Mobile number" className="w-full rounded-xl border border-slate-200 p-3.5 text-sm outline-none transition-all focus:border-orange-500" />
                      </div>
                      <div>
                        <label className="mb-1 block text-[10px] font-bold uppercase tracking-wider text-slate-500">Email ID *</label>
                        <input name="email" required type="email" placeholder="Enter your email id" className="w-full rounded-xl border border-slate-200 p-3.5 text-sm outline-none transition-all focus:border-orange-500" />
                      </div>
                      <div>
                        <label className="mb-1 block text-[10px] font-bold uppercase tracking-wider text-slate-500">Designation *</label>
                        <input name="role" required type="text" placeholder="Enter your designation" className="w-full rounded-xl border border-slate-200 p-3.5 text-sm outline-none transition-all focus:border-orange-500" />
                      </div>
                      <div>
                        <label className="mb-1 block text-[10px] font-bold uppercase tracking-wider text-slate-500">Feedback *</label>
                        <textarea name="content" required placeholder="Write your review…" className="h-28 w-full resize-none rounded-xl border border-slate-200 p-3.5 text-sm outline-none transition-all focus:border-orange-500" />
                      </div>
                    </div>
                    <div className="flex gap-3 pt-2">
                      <button type="submit" className="rounded-xl bg-orange-500 px-8 py-3.5 text-xs font-bold uppercase tracking-wider text-white shadow-md transition-all hover:bg-orange-600">Submit</button>
                    ...
                    <div>
                      ...
                      <div className="group relative flex aspect-[4/3] w-full items-center justify-center overflow-hidden rounded-xl border border-slate-200 bg-slate-50">
                        {avatarPreview ? (
                          ...
                          <div className="flex h-36 w-36 items-center justify-center rounded-full bg-primary-dark text-3xl font-bold text-white/30">VM</div>
                        )}
                        <div className="absolute inset-0 flex items-center justify-center bg-orange-500/10 opacity-0 transition-opacity group-hover:opacity-100"><Plus className="h-8 w-8 text-orange-500" /></div>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.97, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.97, y: 10 }}
                ...
                <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3 md:grid-cols-4">
                  {ALL_INDIA_STATES.map((state) => (
                    <button key={state} onClick={() => { setShowAllStatesModal(false); navigate(`/universities?state=${encodeURIComponent(state)}`); }}
                      className="group flex items-center gap-2.5 rounded-xl border border-transparent bg-slate-50 px-3 py-2.5 text-left transition-all hover:border-orange-500 hover:bg-orange-500/5 hover:text-orange-500 dark:bg-dark-bg">
                      <MapPin className="h-3.5 w-3.5 shrink-0 text-orange-500" />
                      <span className="truncate text-xs font-bold text-slate-700 transition-colors group-hover:text-orange-500 dark:text-slate-300">{state}</span>
                    </button>
                  ))}
                </div>
              </div>
              <div className="flex shrink-0 items-center justify-between border-t border-slate-100 px-6 py-4 dark:border-dark-border">
                ...
                <button onClick={() => { setShowAllStatesModal(false); navigate('/universities'); }}
                  className="rounded-xl bg-orange-500 px-4 py-2 text-xs font-bold uppercase tracking-wider text-white transition-colors hover:bg-orange-600">Browse all</button>
              </div>
            </motion.div>
          </div>
        )}
