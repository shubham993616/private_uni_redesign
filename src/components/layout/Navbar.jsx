...
/**
 * Navbar — MERGED SUPERSET (file 3 styling + file 4 functionality).
 * Sticky, translucent, gains a shadow on scroll. Data-driven mega menus
 * (hover + keyboard focus on desktop, link + accordion on mobile),
 * outside-click / Escape closing, role-aware Admin link, working mobile
 * search. Visuals follow the site's orange theme (plain Tailwind classes,
 * no custom design tokens required).
 */
...
export default function Navbar() {
  ...
  // Close the mobile panel and any open mega menu on navigation —
  // including query-param changes (e.g. clicking between ?stream= links).
  useEffect(() => {
    setMobileOpen(false);
    setActiveDropdown(null);
  }, [location.pathname, location.search]);

  // Outside click + Escape close the mega menus.
  useEffect(() => {
  ...
  return (
    ...
    >
      {/* Relative scope so the mega panels position against the full header width */}
      <div className="relative">
        <Container>
                ...
                return (
                  ...
                  >
                    <Link
                      ...
                      aria-haspopup={l.hasDropdown ? 'menu' : undefined}
                      aria-expanded={l.hasDropdown ? isMenuOpen : undefined}
                      onFocus={() => l.hasDropdown && setActiveDropdown(l.to)}
                    ...
                    {l.hasDropdown && menuData && (
                      <div
                        ...
                        className={`absolute top-full left-4 right-4 mx-auto max-w-7xl bg-white dark:bg-dark-card border border-light-border dark:border-dark-border p-8 shadow-xl rounded-2xl transition-all duration-200 ease-out origin-top z-40 ${
                          isMenuOpen
          ...
          {/* Mobile drawer — working search + navigable links + nested accordions */}
          {mobileOpen && (
                ...
                if (l.hasDropdown && menuData) {
                  return (
                    <div key={l.to} className="space-y-1">
                      {/* Label navigates; chevron toggles the accordion */}
                      <div className="flex items-center">
                        <Link
                          to={l.to}
                          aria-current={isActive(l.to) ? 'page' : undefined}
                          className={`flex-1 flex items-center px-4 h-11 rounded-btn text-sm font-semibold transition-colors ${
                            isActive(l.to)
                        ...
                        <button
                          ...
                          aria-label={`${isAccordionOpen ? 'Collapse' : 'Expand'} ${l.label} menu`}
